...
_IMAGE_FILLER_PATTERNS = (
    r"(你|我|他|她|它|有人|一个|一种)",
    r"(站在|躲在|藏在|蹲在|观察|看着|望着|听见|听到|回荡|注视着|注视|走向|走进|冲向|逃向|出现在|传来|响起)",
    r"(远处|近处|眼前|面前|身后|尽头|门口|后面|前面|旁边)",
    r"(倒塌的|废弃的)",
    r"[的是了着过和与及]",
)
...
def _extract_image_prompt(scene_text: str) -> str:
    text = re.sub(r"[A-Za-z0-9_]+", " ", scene_text or "")
    text = re.sub(r"[，。！？；：,.!?:;\-\n\r\t]+", " ", text)

    for pattern in _IMAGE_FILLER_PATTERNS:
    ...
    for raw_part in text.split(" "):
        part = raw_part.strip("上的里中前后左右内外")
        if len(part) < 2:
            continue
        if part not in seen:
    ...
    for segment in segments:
        ...
        if len(segment) <= remaining:
            ...
            continue
        if remaining >= 2:
            prompt += segment[:remaining]
        break

    return prompt or _truncate_text(scene_text, 10)
...
def _image_prompt(scene_text: str, style_mode: str) -> str:
    return _extract_image_prompt(scene_text)
...
async def _run_image_agent(scene_text: str, style_mode: str = "classic") -> Dict[str, Any]:
...
async def image_agent(scene_text: str, style_mode: str = "classic") -> Dict[str, Any]:
    return await _run_image_agent(scene_text, style_mode)
